...
- Revoke browser-role writes and unused destructive privileges in public tables. Server access and stored rows remain intact. The three new member-record tables are private. Fix the four mutable function search paths.
...
- `__tests__/rewards/security-persistence-rehearsal.mjs`: runs the exact migration in isolated PostgreSQL, checks role restrictions, owner isolation, actual close/reopen persistence, cascade deletion. No production fixtures.
