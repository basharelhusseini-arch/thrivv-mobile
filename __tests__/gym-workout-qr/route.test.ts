...
test('session secret fallback works, while a malformed explicit override fails closed',async()=>{
 (checkGymAccess as jest.Mock).mockResolvedValue({ok:true,user:{id:user}});delete process.env.GYM_WORKOUT_QR_SECRET;expect((await GET(req,context)).status).toBe(200);
 process.env.GYM_WORKOUT_QR_SECRET='short';const r=await GET(req,context);expect(r.status).toBe(503);expect(await r.text()).toContain('complete QR setup');
});
