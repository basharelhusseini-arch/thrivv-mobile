...
test('member sidebar renders the current destinations in order; removed links retain their pages', () => {
  ...
  expect(links).toEqual(['/member/dashboard','/member/workouts','/member/scan-workout','/member/rewards','/member/health','/member/nutrition','/member/bookings','/member/account']);
  expect(desktop).not.toContain('Wearable');
