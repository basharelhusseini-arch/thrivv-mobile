...
test('manual check-in and gym verification are separate required next steps', () => {
  expect(memberNextStep(status()).href).toBe('/member/checkin');
...
test('WHOOP members cannot be routed into manual 40-point check-in by missing workouts', () => {
  ...
  expect(step.href).toBe('/member/whoop'); expect(step.description).toContain('WHOOP workouts only'); expect(step.description).not.toContain('40');
});
