...
const savedReceipt = { id: 'receipt-1', offer_id: 'offer-1', points: 100, status: 'pending', created_at: '2026-09-14T10:00:00Z', reward_offers: { name: 'Existing partner offer' } };
let failedTable = '';
beforeEach(() => {
...
test('saved receipts retain their reference, real offer name, debit and fulfillment status after reload', async () => {
  ...
  expect(reads).toContainEqual({ table: 'reward_offers', field: 'active', value: true });
});
