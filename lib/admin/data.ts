...
export async function memberDetail(id: string) {
  ...
  return { member: member.data, scores: section(results[0]), rewards: section(results[1]), sync: section(results[2]), actions: section(results[3]), membershipHistory: section(results[4]), pointsAdjustments: { available: false, reason: 'Reward accounting must be reconciled before adjustments are enabled.' } };
}
