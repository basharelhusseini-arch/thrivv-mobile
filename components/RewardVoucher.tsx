...
export default function RewardVoucher({ receipt }: { receipt: RewardReceipt }) {
  ...
  return <article className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-6">
    ...
    </div> : <p className="mt-4 text-sm text-thrivv-text-secondary">{expired && receipt.status === 'issued' ? 'This discount code has expired.' : receipt.status === 'pending' ? 'This earlier redemption is awaiting partner confirmation. Contact support with your reference.' : receipt.status === 'fulfilled' ? 'This reward has been fulfilled.' : receipt.status === 'rejected' ? 'The partner reported a problem with this code. Contact support for a replacement or refund.' : receipt.status === 'cancelled' ? 'This redemption was cancelled. Check Point history for any balance adjustment.' : 'Contact support for redemption instructions.'}</p>}
