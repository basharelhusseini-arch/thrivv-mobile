...
export default function DashboardJourney({ activity, rewards, timezone }: { activity: JourneyActivity | null; rewards: JourneyRewards | null; timezone?: string }) {
  ...
  return <section aria-labelledby="journey-title" className="journey-world">
    ...
    <div className="journey-world-header"><h2 id="journey-title"><Sparkles size={15} /> THRIVV JOURNEY</h2><span><span className="journey-live-dot" /> YOUR STORY IN MOTION</span></div>
      ...
      <div className="journey-adventure">
        <div className="journey-identity">
          <div className="journey-orbit" style={{ '--journey-angle': `${journey.percent * 3.6}deg` } as React.CSSProperties}>
            <div className="journey-orbit-core"><LevelIcon size={30} aria-hidden="true" /><span>LEVEL</span><strong>{journey.level.toString().padStart(2, '0')}</strong></div>
            <span className="journey-orbit-spark" aria-hidden="true">✦</span>
          </div>
          <div className="journey-identity-copy"><span className="journey-eyebrow">YOUR CURRENT CHAPTER</span><p className="journey-level-name">{journey.current.name}<span className="text-thrivv-gold-300">.</span></p>
          <p className="journey-caption">{journey.visits === 0 ? 'Every legend starts with a first visit.' : `${journey.visits} verified visit days. Look how far you’ve come.`}</p></div>
          <div className="journey-next"><div><span>{journey.next ? `Next stop: ${journey.next.name}` : 'Legend status achieved'}</span><strong>{journey.next ? `${journey.remaining} visits away` : 'You made it'}</strong></div><Progress value={journey.percent} label={journey.next ? `Progress to ${journey.next.name}` : 'Journey complete'} /></div>
        </div>
        <div className="journey-map-panel">
          <div className="journey-map-heading"><span><Flag size={14} /> THE ROAD TO LEGEND</span><span>Tap a milestone</span></div>
          <ol className="journey-map" aria-label="Visit milestones">{JOURNEY_LEVELS.map((level, index) => {
            const unlocked = journey.visits >= level.visits;
            const BadgeIcon = LEVEL_ICONS[index];
            const current = journey.level === index + 1;
            return <li key={level.name} className={`journey-stop journey-stop-${index} ${unlocked ? 'is-unlocked' : ''} ${current ? 'is-current' : ''}`}>
              <button type="button" className="journey-node" onClick={() => setSelectedMilestone(index)} aria-pressed={(selectedMilestone ?? journey.level - 1) === index} aria-label={`${level.name}, ${level.visits} visit days, ${unlocked ? 'unlocked' : 'locked'}`}>
                {current && <span className="journey-you">YOU ARE HERE</span>}<BadgeIcon size={22} aria-hidden="true" />{unlocked && !current && <span className="journey-earned"><Check size={10} /></span>}
              </button>
              <span className="journey-stop-name">{level.name}</span><span className="journey-stop-count">{level.visits === 0 ? 'Start here' : `${level.visits} days`}</span>
            </li>;
          })}</ol>
          <p className="journey-map-detail" aria-live="polite"><Sparkles size={14} aria-hidden="true" /><span><strong>{selected.name}</strong> · {journey.visits >= selected.visits ? 'Unlocked. This chapter is yours.' : `${selected.visits - journey.visits} more verified visit days to unlock.`}</span></p>
        </div>
      </div>
    ...
    <div className="journey-quests">
      <div className="journey-mission">
        <div className="flex items-center justify-between gap-3"><h3 className="text-sm font-medium text-white"><span className="journey-eyebrow">WEEKLY QUEST</span><br />This week’s mission</h3><span className="text-[10px] uppercase tracking-wider text-thrivv-text-muted">Mon–Sun</span></div>
      ...
      <div className="journey-ticket">
        <h3 className="flex items-center gap-2 text-sm font-medium text-white"><Ticket size={24} className="text-thrivv-gold-400" />{offer && remaining === 0 ? 'Ready to unlock' : 'Your next unlock'}</h3>
