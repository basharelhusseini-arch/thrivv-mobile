  ...
  return <section className="mx-auto max-w-3xl space-y-6" aria-label="Gym workout verification">
    ...
    {data?.manual?.eligible && data.mode !== 'whoop_setup' && data.gymId && <div className="rounded-3xl border border-thrivv-gold-500/20 bg-gradient-to-br from-thrivv-gold-500/[0.06] to-[#0c0e0d] p-6 sm:p-8">
      {data.manual.verified ? <div className="text-center"><CheckCircle2 className="mx-auto mb-4 text-emerald-400" size={36} /><h2 className="text-2xl font-semibold text-white">{data.rewardStatus === 'credited' ? `${data.creditedPoints} points earned` : 'Workout verified'}</h2><p className="mt-3 text-sm text-thrivv-text-secondary">{data.rewardStatus === 'credited' ? 'Today’s workout is complete and your points are in your spendable balance.' : 'Your gym visit is recorded. Check Rewards for the latest credit status.'}</p><Link href="/member/rewards" className="btn-primary mt-6 inline-flex px-6 py-3">View rewards</Link><Link href="/member/checkin" className="mt-4 block text-sm text-thrivv-gold-400">Update today’s habits</Link></div> : <>
        ...
        {!data.manual.enabled && <p className="mt-4 text-sm text-thrivv-text-muted">Gym reward verification is not available right now.</p>}
    ...
    {data?.whoopConnected && <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-6"><h2 className="font-semibold text-white">Your WHOOP workouts</h2><p className="mt-2 text-sm text-thrivv-text-secondary">Optional performance insights. Everyone earns the same attendance points using the scan above; WHOOP syncing does not add another reward.</p>
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs text-thrivv-text-secondary">Today’s Health Score</p><p className="mt-1 text-3xl font-semibold text-thrivv-gold-500">{data.score === null ? '—' : data.score.toFixed(1)}<span className="ml-1 text-xs font-normal text-thrivv-text-muted">/110</span></p></div><button disabled={busy || running} onClick={() => void syncWhoop()} className="btn-primary px-5 py-3 disabled:opacity-50">{busy ? 'Please wait…' : 'Sync workouts'}</button></div>
      {!data.workouts.length && <div className="mt-5 text-sm text-thrivv-text-muted">Your imported workouts will appear here. Record your workout in WHOOP, then tap Sync workouts. <Link href="/member/wearables" className="text-thrivv-gold-400 underline">Manage connection</Link></div>}
      {data.workouts.map(workout => <div key={workout.id} className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-4"><div><h3 className="text-sm font-medium text-white">{workout.sport_name || 'WHOOP workout'}</h3><p className="mt-1 text-xs text-thrivv-text-muted">{workout.date} · {workout.status}</p><p className="mt-2 text-sm text-thrivv-gold-500">Workout score {workout.workout_score == null ? 'pending' : workout.workout_score.toFixed(1)}{workout.end_at && ` · ${Math.max(0, Math.round((Date.parse(workout.end_at) - Date.parse(workout.start_at)) / 60000))} min`}</p></div>{workout.verified && <span className="flex items-center gap-1.5 text-xs text-emerald-400"><CheckCircle2 size={14} />Verified</span>}</div>)}
    </div>}
