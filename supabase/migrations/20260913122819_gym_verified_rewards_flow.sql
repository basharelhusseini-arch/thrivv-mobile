-- Review/isolated rehearsal only. Activation is a separate, forward-only decision.
BEGIN;
...
-- No rounding or rewriting of existing balances. Opening entries have no gym attribution.
INSERT INTO public.reward_transactions(user_id,kind,amount) SELECT id,'opening',coalesce(reward_points,0) FROM public.users;
...
-- Block both table and separately granted column writes to trusted inputs/identity.
-- Existing custom-session server routes retain service_role access.
DO $$ DECLARE t text; c record; BEGIN
 FOREACH t IN ARRAY ARRAY['users','daily_checkins','whoop_workouts','health_score_days','reward_history'] LOOP
