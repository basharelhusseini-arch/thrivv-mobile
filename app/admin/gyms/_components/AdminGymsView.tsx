...
const tabs = ['Overview', 'Gyms', 'Access requests', 'Members', 'Support', 'Rewards', 'Audit history'];
...
function MemberDetails({ id }: { id: string }) {
  ...
  return <section className="premium-card p-5 space-y-5 min-w-0"><LoadState state={state} />{d && <>
    ...
    <h4 className="font-semibold">Gym daily rewards · latest 30 days</h4>{d.dailyRewards?.available ? <ul>{(d.dailyRewards.data || []).map((r:any)=><li key={r.score_date}>{r.score_date}: {r.awarded} credited · {r.status}{r.status==='review_required' ? ` · corrected entitlement ${r.desired}; redemptions paused` : ''}</li>)}</ul> : <p>Daily reward accounting unavailable.</p>}
    <h4 className="font-semibold">Point transactions · latest 30</h4>{d.ledger?.available ? <ul>{(d.ledger.data || []).map((r:any)=><li key={r.id}>{r.score_date || 'Account'}: {r.kind} · {r.amount} points</li>)}</ul> : <p>Point transactions unavailable.</p>}
